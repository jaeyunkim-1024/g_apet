...
<script type="text/javascript">
	$(document).ready(function() {
		var goodsBestYn = '${goodsComment.bestYn}';
...
<div>
	<div class="review-content mt10">
		<div class="review-scroll pbt20">
			<%-- <p class="pbt20"><img class="imgRedius" src="${petlogVo.imgPath}" > ${petlogVo.petlogEstmId} </p> --%>
			<c:choose>
				<c:when test="${empty goodsComment.vdPath or goodsComment.vdPath == ''}">
					<div class="slideshow-container">
						<!-- Slideshow container -->
						<div class="slideshow">
							<c:forEach items="${goodsComment.imgPathList}" var="img" varStatus="st">
								<div class="mySlides fade">
									<div class="numbertext">${st.count} / ${fn:length(goodsComment.imgPathList)}</div>
									<%-- <img src="<frame:imgUrl/>${img.imgPath}" style="width:100%;height: 350px;"> --%>
									<img src="${fn:indexOf(img, 'cdn.ntruss.com') > -1 ? img : frame:optImagePath(img, adminConstants.IMG_OPT_QRY_560)}" style="width:100%;height: 350px;">	
								</div>
							</c:forEach>
							<br>
							<div class="center">
							...
							<c:if test="${not empty goodsComment.imgPathList and fn:length(goodsComment.imgPathList) > 1 }">
				...
				<c:when test="${not empty goodsComment.vdPath and goodsComment.vdPath != ''}">
					<iframe src ="<spring:eval expression="@bizConfig['vod.player.api.url']" />/load/${goodsComment.vdPath}?v=1&vtype=mp4"
							frameborder="0"
							width="700px"
							height="350px"
							scrolling="no"
			...
			<div style="margin-left: 10px">
				<c:out value="${goodsComment.dscrt}" />
			...
			<tbody>
				...
				<c:forEach items="${rptpRsnInfoListVO}" var="rptrRsnInfoList" varStatus="status">
					<tr>
						...
						<td class="center">${rptrRsnInfoList.rptpContent }</td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
	...
	<div class="panel-title" id="mngTitle">노출관리</div>
		<form id="goodsCommentForm" name="goodsCommentForm" method="post" >
			<input type="hidden" id="goodsEstmNo" name="goodsEstmNo" value="${goodsComment.goodsEstmNo }" />
			<input type="hidden" id="goodsBestYn" name="goodsBestYn" value="${goodsComment.bestYn }" />
			<table class="table_type1" style="bottom: 5px;">
				<colgroup>
					<col style="width:100px;" />
					<col />
					<col style="width:100px;" />
