	...
	<t:putAttribute name="script">
		...
		<script type="text/javascript">
			...
			function createOrderListGrid(){
				...
				var options = $.extend( {} ,  {
					url : "<spring:url value='/claim/claimReturnListGrid.do' />"
					, height : 400
					,colModels : [
					 			// 클레임 번호
					 			...
					 			// 클레임 상세 상태 코드
					 			, {name:"clmDtlStatCd", label:'<spring:message code="column.clm_dtl_stat_cd" />', width:"80", align:"center", formatter:"select", editoptions:{value:"<frame:gridSelect grpCd="${adminConstants.CLM_DTL_STAT}" usrDfn2Val="20" />"}}
					 			// 클레임 사유 코드
					...
					, gridComplete: function() {  /** 데이터 로딩시 함수 **/
						...
						var ids = $('#clmReturnList').jqGrid('getDataIDs');

						// 그리드 데이터 가져오기
						var gridData = $("#clmReturnList").jqGrid('getRowData');
						...
						if(gridData.length > 0){
							...
							for (var i = 0; i < gridData.length; i++) {
								...
								if (gridData[i].clmStatCd == '40') {
									...
									$('#clmReturnList tr[id=' + ids[i] + ']').css('color', 'red');
							   }
							}
						}
					...
					,onCellSelect: function(rowid, index, contents, e){ // cell을 클릭 시
						...
						var cm = $("#clmReturnList").jqGrid("getGridParam", "colModel");
						var rowData = $("#clmReturnList").getRowData(rowid);
				...
				grid.create( "clmReturnList", options) ;

			}

			// 클레임상세
			function fnClaimDetailView(clmNo) {
				addTab('클레임 상세', '/return/claimDetailView.do?clmNo=' + clmNo);
			}

			// 그리드 체크 박스 클릭 : 클레임 단위 토글
			$(document).on("click", "#clmReturnList input[name=arrClmNo]", function(e){
// 				alert( $(this).val() );
				var checked = $(this).is(":checked");
				$("#clmReturnList input[name=arrClmNo]").prop("checked", false);
				$(this).prop("checked", checked);
			});

			// 주문상세
			function fnOrderDetailView(ordNo) {
				addTab('주문 상세', '/return/orderDetailView.do?ordNo=' + ordNo + "&viewGb=" + '${adminConstants.VIEW_GB_POP}');
			}
			...
			function reloadOrderListGrid() {
				...
				gridReload('clmAcptDtmStart','clmAcptDtmEnd','clmReturnList','term', options);	
			}
...
<c:if test="${adminConstants.USR_GB_2010 eq adminSession.usrGbCd}">
			...
			// 엑셀 다운로드
			function claimReturnListExcelDownload() {
				var data = $("#claimSearchForm").serializeJson();
				var rowIds = $("#clmReturnList").getGridParam("selarrrow");
				if(rowIds.length > 0){
					var clmNos = [];
					for(var i in rowIds){
						var rowData =  $("#clmReturnList").getRowData(rowIds[i]);
				...
				createFormSubmit( "claimReturnListExcelDownload", "/claim/claimReturnListExcelDownload.do", data );
			}
							...
							<tr>
		                        ...
		                        <td>
		                        	<select name="clmDtlTpCd" id="clmDtlTpCd" class="wth100" title="선택상자" >
										<frame:select grpCd="${adminConstants.CLM_DTL_TP }" selectKey="${adminConstants.CLM_DTL_TP_20}" readOnly="true"  />
									</select>
									 
									<select name="clmDtlStatCd" id="clmDtlStatCd" class="wth100" title="선택상자" >
										<frame:select grpCd="${adminConstants.CLM_DTL_STAT }" usrDfn1Val="${adminConstants.CLM_TP_20}"  defaultName="전체" />
									</select>
		                        </td>
			                    <!-- 주문정보 -->
		...
		<div class="mModule">
			<div class="mButton">
				<div class="leftInner">
					...
					<button type="button" onclick="fnClaimProductRecoveryFinal();" class="btn btn-add"><!-- 반품회수완료 -->
					...
					<c:if test="${adminConstants.USR_GRP_10 eq session.usrGrpCd}" >
						<button type="button" onclick="fnClaimProductApprove();" class="btn btn-add"><!-- 반품승인완료  -->
							<spring:message code='column.order_common.btn.claim_return_product_approve_final' />
						</button>
						<button type="button" onclick="fnClaimProductRejectionFinal();" class="btn btn-add"><!-- 반품승인거부  -->
							<spring:message code='column.order_common.btn.claim_return_product_approve_refuse' />
						</button>
					</c:if>
				</div>
				<div class="rightInner">
					<button type="button" onclick="claimReturnListExcelDownload();" class="btn btn-add btn-excel"><spring:message code='column.common.btn.excel_download' /></button>
				</div>
			</div>
			
			<table id="clmReturnList" ></table>
			<div id="clmReturnListPage"></div>
		</div>
		<!-- ==================================================================== -->
		<!-- //그리드 -->
		<!-- ==================================================================== -->
