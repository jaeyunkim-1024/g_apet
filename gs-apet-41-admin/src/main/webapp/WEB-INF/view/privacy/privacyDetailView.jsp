...
<t:insertDefinition name="contentsLayout">
	<t:putAttribute name="script">
		...
		function updatePrivacy () {
			if(validate.check("privacyUpdateForm")) {
				oEditors.getById["content"].exec("UPDATE_CONTENTS_FIELD", []);
				// 내용 체크
				if( !editorRequired( 'content' ) ){ return false };
				
				var formData = $("#privacyUpdateForm").serializeJson();
				...
				messager.confirm("<spring:message code='column.common.confirm.update' />", function(r){
					if(r){
						var options = {
							url : "<spring:url value='/privacy/privacyUpdate.do' />"
							, data : sendData
	...
	<t:putAttribute name="content">
		<form id="privacyUpdateForm" name="privacyUpdateForm" method="post" >
			<table class="table_type1">
				...
				<tbody>
					<tr>
						...
						<td>
							<input type="hidden" id="privacyNo" name="policyNo" title="<spring:message code="column.policy_no" />" value="${privacyPolicy.policyNo }" />
							<b>자동입력</b>
						</td>
						<th scope="row"><spring:message code="column.version_info" /><strong class="red">*</strong></th>	<!-- 버전 정보 -->
						<td>
							<input type="text" class="w300 validate[required, maxSize[50]]" id="versionInfo" name="verInfo" title="<spring:message code="column.version_info" />" value="${privacyPolicy.versionInfo }" />
						</td>
					</tr>
					<tr>
						...
						<td>
							<!-- 사이트 ID-->
							<frame:stId funcNm="searchSt()" requireYn="Y" defaultStNm="${privacyPolicy.stNm }" defaultStId="${privacyPolicy.stId }" />
						</td>
						<th scope="row"><spring:message code="column.use_yn" /></th>	<!-- 사용 여부 -->
						<td>
							<frame:radio name="useYn" grpCd="${adminConstants.USE_YN }" selectKey="${privacyPolicy.useYn }" />
						</td>
					</tr>
					<tr>
						<th scope="row"><spring:message code="column.content" /><strong class="red">*</strong></th>	<!-- 내용 -->
						<td colspan="3">
							<textarea name="content" id="content" class="validate[required]" cols="30" rows="10" style="width:100%;height:500px;">${privacyPolicy.content }</textarea>
						</td>
					</tr>
				</tbody>
			</table>
		...
		<div class="btn_area_center">
			<button type="button" class="btn btn-ok" onclick="updatePrivacy();" >수정</button>
