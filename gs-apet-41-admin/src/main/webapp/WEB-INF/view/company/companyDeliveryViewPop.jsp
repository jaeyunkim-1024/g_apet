<%@ page pageEncoding="UTF-8" contentType="text/html; charset=utf-8" %>
		...
		function insertCompanyDelivery() {
			if(validate.check("companyDeliveryViewForm")) {
				messager.confirm('<spring:message code="column.common.confirm.insert" />', function(r){
	                if (r){
	                	var options = {
    						url : "<spring:url value='/company/companyDeliverySave.do' />"
    						, data : $("#companyDeliveryViewForm").serializeJson()
    						, callBack : function(result){
    							...
    							<c:choose>
    								...
    								<c:otherwise>
    									var msg = "<spring:message code='admin.web.view.msg.company.delivery.insert_final_msg' />";
    								</c:otherwise>
    							</c:choose>
    							
    							messager.alert(msg, "Info", "info", function(){ 
    								reloadCompanyDeliveryGrid();
    								layer.close('companyDeliveryView');
    							});
