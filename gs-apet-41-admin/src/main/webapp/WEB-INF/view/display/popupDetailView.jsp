...
<t:insertDefinition name="contentsLayout">
	<t:putAttribute name="script">
		...
		$(document).ready(function() {
			...
			<c:if test="${adminConstants.POPUP_TP_20 eq popup.popupTpCd}">
			$("#goodsView").show();
			grid.resize();
			</c:if>
		...
		function createpopupDispGrid () {
			var options = {
				url : "<spring:url value='/display/popupShowDispClsfGrid.do' />"
				, searchParam : {popupNo : '${popup.popupNo}' }
				, paging : false
				, cellEdit : true
				, height : 150
				, colModels : [
		...
		function updatePopup () {
			...
			if(validate.check("popupUpdateForm")) {
				...
				var formData = $("#popupUpdateForm").serializeJson();
				...
				messager.confirm("<spring:message code='column.common.confirm.update' />",function(r){
					if(r){
						var options = {
								url : "<spring:url value='/display/popupUpdate.do' />"
								, data : sendData
		...
		function displayCategoryAddPop() {
			// 사이트 선택값
			var stIdVal = $("#stIdCombo").val();
			// 전시카테고리 선택값
			var dispClsfCdVal = $("#dispClsfCdCombo").val();
...
<c:otherwise>
						...
						<tr>
                              ...
                              <td>
                                   <input type="text" class="readonly" id="goodsId" name="goodsId" title="상품ID" value="${popup.goodsId }" readonly>
                                   &nbsp;
...
<div id="goodsView" style="display: none;" >
			...
			<div class="mModule no_m">
				...
				<div class="btn_area_center">
					<button type="button" class="btn btn-ok" onclick="updatePopup(); return false;" >수정</button>
