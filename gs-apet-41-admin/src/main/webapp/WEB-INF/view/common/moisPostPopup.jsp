...
<% 
	...
	//String confmKey = "devU01TX0FVVEgyMDIwMTIyOTE0MjI1NzExMDYwNzE="; // 검색API 승인키
...
<script type="text/javascript">
		...
		$.ajax({
			...
			,success:function(xmlStr){
				...
				if( parseInt(totalCount) > 1000 && currentPage == "1" )
					messager.alert("검색 결과가 너무 많습니다(1,000건 이상)\n검색어 예를 참조하여 검색하시기 바랍니다.", "Info", "info");
		    ...
		    ,error: function(xhr,status, error){
		    	messager.alert("검색에 실패하였습니다 \n 다시 검색하시기 바랍니다.", "Error", "error");
		    }
		});
	...
	function addrDetailChk(){
		...
		if(evtCode == 63 || evtCode == 35 || evtCode == 38 || evtCode == 43 || evtCode == 92 || evtCode == 34){ // # & + \ " 문자제한
			messager.alert('특수문자 ? # & + \\ " 를 입력 할 수 없습니다.', "Info", "info");
...
<form name="rtForm" id="rtForm" method="post">
	...
	<input type="hidden" name="roadFullAddr" id="roadFullAddr"/> <!-- 전체 도로명주소(주소+상세주소) [서울특별시 강남구 논현로 508, 지하1층 (역삼동)] -->
	<input type="hidden" name="roadAddrPart1" id="roadAddrPart1"/> <!-- 도로명주소 [서울특별시 강남구 논현로 508] -->
	<input type="hidden" name="roadAddrPart2" id="roadAddrPart2"/> <!-- 참고주소 [(역삼동)] -->
...
<form name="AKCFrm" id="AKCFrm" method="post">
	...
	<input type="hidden" name="confmKey" value="<spring:eval expression="@bizConfig['bo.mois.post.confmKey']" />"/> <!-- 신청시 발급받은 승인키 -->
	...
	<input type="hidden" name="rtRoadAddr"  id="rtRoadAddr"  /> <!-- 전체 도로명주소 [서울특별시 강남구 논현로 508 (역삼동)] -->
	<input type="hidden" name="rtAddrPart1" id="rtAddrPart1" /> <!-- 도로명주소 [서울특별시 강남구 논현로 508] -->
	<input type="hidden" name="rtAddrPart2" id="rtAddrPart2" /> <!-- 참고주소 [(역삼동)] -->
