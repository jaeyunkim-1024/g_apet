...
let rIndex = 0;
var isGridExistsConts = false;
if(isGridExistsConts){
	var options = {
		searchParam : $("#umTagListContsForm").serializeJson()
	};
	grid.reload("umTagListConts", options);
}else{
	viewNewTagRltListPopGrid4();
}

function viewNewTagRltListPopGrid4(){			
	setLayerPopParam("Conts");
	
	var gridOptions = {
			url : "<spring:url value='/tag/pageUmTagContsLayer.do' />"
			, height : 300
			, searchParam : $("#umTagListContsForm").serializeJson()
			, sortname : 'sysRegDtm'
			, sortorder : 'DESC'
			, colModels : [
				{name:"rowIndex", label:'No.', width:"55", align:"center", classes:'cursor_default', sortable:false, formatter: function(cellvalue, options, rowObject){
					let nTotalCnt = $("#umTagListConts").getGridParam("records");
					let nPage = $('#umTagListConts').jqGrid('getGridParam', 'page');
					let nRows = $('#umTagListConts').jqGrid('getGridParam', 'rowNum');
			...
			, onCellSelect : function (id, cellidx, cellvalue) {
				if(cellidx == 3) {
					var rowData = $("#umTagListConts").getRowData(id);
					var rltId = rowData.rltId;					
					var url ='https://<spring:eval expression="@webConfig['site.fo.domain']" />/tv/series/indexTvDetail?vdId='+rltId+'&sortCd=&listGb=HOME'; 
			...
			, gridComplete : function() {
				 $("#noData").remove();
				var grid = $("#umTagListConts").jqGrid('getRowData');
				if(grid.length <= 0) {
					var str = "";
					...
					$("#umTagListConts.ui-jqgrid-btable").append(str);
				}
			}
		}
	grid.create("umTagListConts", gridOptions);
...
<form id="umTagListContsForm" name="umTagListContsForm" method="post" >
...
<br/>
<br/>
<h2 style="padding-left:10px">▶ 관련 영상</h2>
<div title="관련 상품" style="padding:10px">
	<table id="umTagListConts" ></table>
	<div id="umTagListContsPage"></div>				
</div>
