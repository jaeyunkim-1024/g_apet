	...
	<t:putAttribute name="script">
		...
		<script type="text/javascript">
			...
			var claimList = {
				...
				createGrid : function(){
					var options = {
						url : "<spring:url value='/market/claimExchangeListGrid.do' />"
						, colModels : [
                         	// 처리상태
			...
			// 교환등록 처리
			function dcgClmExchangeMake() {
				var grid = $("#claimList");
				var selectedIDs = grid.getGridParam("selarrrow");
				
				if(selectedIDs != null && selectedIDs.length > 0) {
					
					messager.confirm("<spring:message code='column.order_common.confirm.ob_claim_exchange_final' />",function(r){
						if(r){
							var data = new Array();
							...
							var options = {
								url : "<spring:url value='/market/marketClaimExchangeConfirm.do' />"
								, data : {
									arrClmSeq : data.join(",")
								}
			...
			<table summary="">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<frame:checkbox name="arrClmStatCd" grpCd="${adminConstants.MARKET_CLM_STAT }" defaultName="전체" defaultId="arrClmStatCd_default" excludeOption="${adminConstants.MARKET_CLM_STAT_01},${adminConstants.MARKET_CLM_STAT_02},${adminConstants.MARKET_CLM_STAT_105},${adminConstants.MARKET_CLM_STAT_106}" />
						</td>
					</tr>										
				</tbody>
			</table>
...
<c:if test="${adminConstants.USR_GRP_10 eq adminSession.usrGrpCd}">			
				<button type="button" onClick="dcgClmExchangeMake();" class=btn_h25_type1>교환등록 처리</button>
</c:if>				
			</div>
		</div>
		<!-- ==================================================================== -->
