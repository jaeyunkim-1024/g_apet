	...
	<t:putAttribute name="script">
		...
		<script type="text/javascript">
			...
			var claimList = {
				...
				createGrid : function(){
					var options = {
						url : "<spring:url value='/market/claimCancelListGrid.do' />"
						, colModels : [
                         	// 처리상태
			...
			// 취소완료 처리
			function dcgClmCancelMake() {
				var grid = $("#claimList");
				var selectedIDs = grid.getGridParam("selarrrow");
				
				if(selectedIDs != null && selectedIDs.length > 0) {
					messager.confirm("<spring:message code='column.order_common.confirm.ob_claim_cancel_final' />", function(r){
						if(r){
							var data = new Array();
							...
							var options = {
								url : "<spring:url value='/market/marketClaimCancelConfirm.do' />"
								, data : {
									arrClmSeq : data.join(",")
								}
			...
			<table summary="">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<frame:checkbox name="arrClmStatCd" grpCd="${adminConstants.MARKET_CLM_STAT }" defaultName="전체" defaultId="arrClmStatCd_default" excludeOption="${adminConstants.MARKET_CLM_STAT_105},${adminConstants.MARKET_CLM_STAT_106},${adminConstants.MARKET_CLM_STAT_201},${adminConstants.MARKET_CLM_STAT_212}" />
						</td>
					</tr>										
				</tbody>
			</table>
...
<c:if test="${adminConstants.USR_GRP_10 eq adminSession.usrGrpCd}">			
				<button type="button" onClick="dcgClmCancelMake();" class=btn_h25_type1>취소완료 처리</button>
</c:if>				
			</div>
		</div>
		<!-- ==================================================================== -->
