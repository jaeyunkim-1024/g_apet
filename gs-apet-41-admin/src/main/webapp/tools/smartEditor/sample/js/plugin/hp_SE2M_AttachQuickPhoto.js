...
nhn.husky.SE2M_AttachQuickPhoto = jindo.$Class({
	...
	$init : function(oAppContainer, bUsePhoto){
		this._assignHTMLObjects(oAppContainer, bUsePhoto);
	},

	_assignHTMLObjects : function(oAppContainer, bUsePhoto){
		oAppContainer = jindo.$(oAppContainer) || document;
	
		this.photoArea = cssquery.getSingle(".se2_multy", oAppContainer);
		
		//설정이 없거나, 사용하겠다고 표시한 경우 block 처리
		if( typeof(bUsePhoto) == 'undefined' || bUsePhoto === true){
			this.photoArea.style.display = "block";
		}else{
			this.photoArea.style.display = "none";		
		}
	},
...
//		this.oApp.htOptions
		...
		// 카피온 수정
		parent.fileUpload.editorImage(this.oApp.elPlaceHolder.name, this.oApp.htOptions.imgPath);
	},
