...
public class NanoHTTPD
{
	...
	public class Response
	{
		...
		public Response( String status, String mimeType, String txt )
		{
			...
			try {
				bis = new ByteArrayInputStream( txt.getBytes());
				this.status = status;
				this.mimeType = mimeType;
				this.data = bis;
			} finally {
	...
	public void stop()
	{
		...
		catch ( IOException ioe ) {log.error("IOException");}
		catch ( InterruptedException e ) {log.error("InterruptedException");}
		finally {//보안 진단. 부적절한 자원 해제 (IO)
			if(myServerSocket.isBound() && !myServerSocket.isClosed()) {
				myServerSocket.close();
			}
		}
	}
 ...
 new File("").getAbsolutePath() + "\"" );
		...
		try { System.in.read(); } catch( Throwable t ) {log.error("Throwable");};
	}
	...
	private class HTTPSession implements Runnable
	{
		...
		public void run()
		{
				...
				try {
					...
					if ( method.equalsIgnoreCase( "POST" ))
					{
						...
						if (contentLength != null)
						{
							...
							catch (NumberFormatException ex) {log.error("NumberFormatException");}
						}
						String postLine = "";
			...
			catch ( IOException ioe )
			{
				...
				catch ( Throwable t ) {log.error("Throwable");}
			}
		...
		private void sendError( String status, String msg ) throws InterruptedException
		{
			...
			try {
				...
				sendResponse( status, MIME_PLAINTEXT, null, bis);
		...
		private void sendResponse( String status, String mime, Properties header, InputStream data )
		{
			try
			{
				...
				try {
					...
					pw.print("HTTP/1.0 " + status.replaceAll("[-<>,!/\"'%;\\(\\)&[+]]","") + " \r\n");

					if ( mime != null ) {
						pw.print("Content-Type: " + mime.replaceAll("[-<>,!/\"'%;\\(\\)&[+]]","") + "\r\n");
					}
			...
			catch( IOException ioe )
			{
				...
				try { mySocket.close(); } catch( Throwable t ) {log.error("Throwable");}
			}
		}
		...
		try
		{
			...
			if ( range != null )
			{
				if ( range.startsWith( "bytes=" ))
				{
					...
					catch ( NumberFormatException nfe ) {log.error("NumberFormatException");}
				}
			}
			//보안 진단. 부적절한 자원 해제 (IO)
