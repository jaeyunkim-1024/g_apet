			...
			form.find('['+options.validateAttribute+'*=validate]').not(":disabled").each( function() {
				...
				if ($.inArray(field.attr('name'), names) < 0) {
					...
					if (errorFound && first_err==null)
						// 2020.02.10 김진홍 - textarea Editor 적용 시 field 체크함. textarea에 update 후 체크해야함. 
						if(field.is(":hidden") && field.prop("type") == "textarea" && field.next("iframe").length > 0){
							
							first_err=field;
						}
						else if (field.is(":hidden") && options.prettySelect){
		...
		*/
		_buildPrompt: function(field, promptText, type, ajaxed, options) {
			...
			/*if(positionType === 'inline') {
				...
				field.before(prompt);				
			} validation div 생성 19.05.07 이전 원본 소스 유준희 */
			
			/* validation div input 뒤에 생성되도록 변경 19.05.07 유준희 */
			//field.after(prompt);
			
			if(field.is(':radio') || field.is(':checkbox')){
				/*
				 * design.css - label:not(:last-child) .formError {display: none} 삭제 or 아래 처리
				 * table td 안에서 radio 필수 체크, 20.12.31 김진홍
				 * */
				if(field.closest('td').length > 0){
					field.closest('td').append(prompt);
				}else{
					field.parent().append(prompt);
				}
			} else if(field.next('button').length) {
				field.next('button').after(prompt);
			} else {
				field.parent().append(prompt);
			}
