		...
		sld:{ // 샵 메인 슬라이드UI 모음
			...
			often:{ // 자주구매한상품 
				...
				opt: {
					...
					// APETQA-6313 210914 lju02 - 모바일에서는 강제값을 주기때문에 BP제어가 필요 없어서 삭제
					/*breakpoints: {
						1024: {
							spaceBetween:12,
							slidesPerView: "auto",
		...
		plmov:{ // 기획전 팝업 스크롤이동 Shop_02_01.html
			...
			mov:function(id){
				setTimeout(function(){
					...
					$("body,html").animate({ scrollTop: scr_top - 110 }, 200, function(){  });	/* APETQA-4733 210727 lju02 */
				...
				$el.each(function(idx,n){
					...
					if($(n).hasClass("hmode_auto") && $("link[href *= 'style.mo.css']").length > 0){
						setTimeout(function() {				// 화면 그려주는 순서 문제로 settimeout으로 묶어줌 - APETQA-5598 210806 lju02 
							...
							$box.children().eq(0).height(setH);		// 초기 .uiTab_content > ui 높이
						},500);
					}
			...
			setTimeout(function(){
				...
				if(check) {
					...
					$(window).scroll(function(e){
						...
						if($(".best_category_wrap").length){		//베스트20 신규 코드용 - APET-1129 210713 lju02
...
function callTabSwiperPc(){	/* APET-1227 210730 lju02 */
	setTimeout(function() {
		...
		$('.tabSwiper .swiper-slide').css({"min-height": slidH});
	...
	$(document).on("click", ".my-accd .qalist>li .hBox .tit", function() {	// 아코디언 클릭시 스와이퍼 자동 높이 조정 호출
		setTimeout(function() {
