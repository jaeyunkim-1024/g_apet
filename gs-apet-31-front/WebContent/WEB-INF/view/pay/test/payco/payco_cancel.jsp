...
<%
	...
	String cancelType 						= (String)request.getParameter("cancelType").replaceAll(xssCheckStr,"");						//취소 Type 받기 - ALL 또는 PART
	String orderNo							= (String)request.getParameter("orderNo").replaceAll(xssCheckStr,"");							//PAYCO에서 발급받은 주문번호
	String orderCertifyKey					= (String)request.getParameter("orderCertifyKey").replaceAll(xssCheckStr,"");					//PAYCO에서 발급받은 주문인증 key
	String cancelTotalAmt 					= (String)request.getParameter("cancelTotalAmt").replaceAll(xssCheckStr,"");					//총 취소 금액
	String totalCancelTaxfreeAmt			= (String)request.getParameter("totalCancelTaxfreeAmt").replaceAll(xssCheckStr,"");			//주문 총 면세금액
	String totalCancelTaxableAmt			= (String)request.getParameter("totalCancelTaxableAmt").replaceAll(xssCheckStr,"");			//주문 총 과세 공급가액
	String totalCancelVatAmt				= (String)request.getParameter("totalCancelVatAmt").replaceAll(xssCheckStr,"");				//주문 총 과세 부가세액
	String totalCancelPossibleAmt			= (String)request.getParameter("totalCancelPossibleAmt").replaceAll(xssCheckStr,"");			//총 취소가능금액
	String sellerOrderProductReferenceKey 	= (String)request.getParameter("sellerOrderProductReferenceKey").replaceAll(xssCheckStr,"");	//가맹점 주문 상품 연동 키(PART 취소 시)
	String cancelDetailContent 				= (String)request.getParameter("cancelDetailContent").replaceAll(xssCheckStr,"");				//취소 상세 사유
	String cancelAmt 						= (String)request.getParameter("cancelAmt").replaceAll(xssCheckStr,"");						//취소 상품 금액(PART 취소 시)
	String requestMemo						= (String)request.getParameter("requestMemo").replaceAll(xssCheckStr,"");						//취소처리 요청메모
	...
	try{
		...
		//e.printStackTrace();
		System.out.println("Exception when payco_cancel");
