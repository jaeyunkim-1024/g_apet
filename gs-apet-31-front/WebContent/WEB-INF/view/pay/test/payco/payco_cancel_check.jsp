...
<%
	...
	String orderNo							= (String)request.getParameter("orderNo").replaceAll(xssCheckStr,"");							//[필수]PAYCO에서 발급받은 주문번호
	String cancelTotalAmt 					= (String)request.getParameter("cancelTotalAmt").replaceAll(xssCheckStr,"");					//[필수]총 취소 금액
	String sellerOrderProductReferenceKey 	= (String)request.getParameter("sellerOrderProductReferenceKey").replaceAll(xssCheckStr,"");	//[필수]가맹점 주문 상품 연동 키(PART 취소 시)
	String cancelAmt 						= (String)request.getParameter("productAmt").replaceAll(xssCheckStr,"");						//[필수]취소 상품 금액(PART 취소 시)
	...
	if(orderNo == null || orderNo.equals("")){
		...
		try{
			...
			//e.printStackTrace();
			System.out.println("Exception when payco_cancel_check");
		}
