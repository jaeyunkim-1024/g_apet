...
<script type="text/javascript">
	$(function(){
		...
		$("#writeQna").on('click', function(){
			if(goodsQna.mbrNo == 0){
				location.href = "/indexLogin?"+encodeURIComponent(document.URL);
			}else{
		...
		$(".layers").on('click', '#popQnaMod .btnPopClose', function(){
			ui.confirm('문의 내용 작성을 취소하시겠습니까?',{ // 컨펌 창 옵션들
				ycb:function(){
					$("#popQnaMod").remove();
				},
				ncb:function(){
	...
	var goodsQna = {
		...
		insertGoodsQna : function(){
			...
			if($("textarea[name=iqrContent]").hasClass('disabled')){
				return false;
			}
			var addData = {
				goodsId : goodsQna.goodsId
...
<div class="cdts">
	<div class="uiqnalist">
		<div class="secret"><label class="checkbox"><input type="checkbox" id="hiddenCheck"><span class="txt"><em class="tt">비밀글 제외</em></span></label></div>
