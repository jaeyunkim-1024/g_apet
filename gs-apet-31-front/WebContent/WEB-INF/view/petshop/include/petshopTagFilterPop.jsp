...
$(document).ready(function(){
	//상세조건 선택
	$("div[id=goodsTag]>button[name=filList").click(function(){
...
var filtDetail = {
		...
		brand : function(sidx, sord, param){	
			//APETQA-5584
			$("#sortId li[class=active]").removeClass("active");
			if(filter.brandSort == "v_1"){
				$("button[value=v_1]").parent("li").addClass("active");	
				$("#popSortNm").text($("button[value=v_1]").text());
			}else if(filter.brandSort == "v_2"){
				$("button[value=v_2]").parent("li").addClass("active");
				$("#popSortNm").text($("button[value=v_2]").text());
			}
			if(param != "btn"){
...
// 		 			dispClsfNo : dispClsfNo,
		...
		,count : function(){
			...
			var options = {
					...
					data : {
						dispClsfNo : '${frontConstants.DISP_CLSF_10}', 
						filters : filters,
						bndNos : bndNos,
						tags : tags
					}
		...
		,append : function(id, text, param){
			...
			if(param == "fil" && $("button[id="+id+"]").attr('class') == 'active'){
			...
			<section class="sect k0426" data-sid="ui-tabs">
				...
				<div data-ui-tab-ctn="tab_a" data-ui-tab-val="tab_a_1" class="active">
					<div class="filter-area">
						<c:forEach items="${attrNm}" var="attrNm">
							<div class="filter-item">
							<strong class="tit">${attrNm}</strong>
								<div class="tag" id="goodsTag">
									<c:forEach items="${filters}" var="filter">
										<c:if test="${attrNm eq filter.filtGrpMngNm}">
											<button id="${filter.filtId}" name="filList" class="" >${filter.filtAttrNm}</button>
	 									</c:if> 
									</c:forEach>a
								</div>
							</div>
				...
				<div data-ui-tab-ctn="tab_a" data-ui-tab-val="tab_a_2">
					<div class="brand-check">
						<nav class="uisort only_down">
							<button type="button" class="bt st" id="popSortNm">인기순</button>
							<div class="list">
								<ul class="menu" id="sortId">
									<li><button name="sortBtn" type="button" class="bt" value="v_1">인기순</button></li>
									<li><button name="sortBtn" type="button" class="bt" value="v_2">가나다순</button></li>
								</ul>
							</div>
						</nav>
						<ul class="brand-list">
