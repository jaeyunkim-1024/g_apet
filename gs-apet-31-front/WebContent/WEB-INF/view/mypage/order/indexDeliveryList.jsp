...
<script type="text/javascript">
		...
		$(document).ready(function(){
			if($("#noDataDeliveryList").length>0){
				waiting.start();
				$("#noDataDeliveryList").hide();
				setTimeout(function(){
					$("#noDataDeliveryList").show();
					waiting.stop();
				},1000);
			}
...
// 			//결제완료시 전체주문번호 노출
		...
		var orderDeliveryList = {
			...
			,searchStauts : function(stus, clickId){
				...
				var options = {
						url : "<spring:url value='/mypage/order/ajaxDeliveryHtml' />"
						,data : formData	
						, dataType: 'html'
						,done : function(data) {
							let noDataHeight = "";
							...
							$("#deliveryList .inr-box.piner")[1].remove();
			...
			, scrollPaging  : function() {			
				...
				if (callAjaxdeliveryHtml == 'N' && Number(nowPage) < Number(endPage) ){
					...
					var formData = $("#delivery_list_form").serialize();				
					var options = {
							url : "<spring:url value='/mypage/order/ajaxDeliveryListHtml' />"
							,data : formData	
							, dataType: 'html'
							,done : function(data) {
								...
								$("#deliveryList").append(data);
								$("#deliveryList .inr-box.piner").last().remove();
		...
		function orderStatusChangeFn(statusClass) {
		    ...
		    switch (statusClass){
				case "110" :
					...
					$(".btn_round").show();
					$(".btn_fixed_wrap").css("display","flex");
					break;
				case "120" :
					ui.nodata_set.init();
					$("#stepTwo").addClass("on");
							...
							<div class="pc-re-po01">
								<div class="oder-step ptb_memOrderbox">
									...
									<nav class="menushop re-po01">
										...
										<div class="list">
											<ul class="menu">
												<li <c:if test="${orderSO.period eq '3'}">class="active"</c:if> ><a href="javascript:void(0);"" id="period_type_3" data-content="${session.mbrNo }" data-url="/mypage/order/indexDeliveryList?page=${orderSO.page}&rows=${orderSO.rows}&period=3&ordAcptDtmStart=<frame:timestamp date="${orderSO.ordAcptDtmStart}" dType="H" />&ordAcptDtmEnd=<frame:timestamp date="${orderSO.ordAcptDtmEnd}" dType="H" />&arrOrdDtlStatCd=${ orderSO.arrOrdDtlStatCd }" class="bt" onclick="orderDeliveryList.setPeriod(3);return false;">최근 3개월</a></li>
												<li <c:if test="${orderSO.period eq '6'}">class="active"</c:if>><a href="javascript:void(0);"" id="period_type_6" data-content="${session.mbrNo }" data-url="/mypage/order/indexDeliveryList?page=${orderSO.page}&rows=${orderSO.rows}&period=6&ordAcptDtmStart=<frame:timestamp date="${orderSO.ordAcptDtmStart}" dType="H" />&ordAcptDtmEnd=<frame:timestamp date="${orderSO.ordAcptDtmEnd}" dType="H" />&arrOrdDtlStatCd=${ orderSO.arrOrdDtlStatCd }"	class="bt" onclick="orderDeliveryList.setPeriod(6);return false;">최근 6개월</a></li>
												<li <c:if test="${orderSO.period eq '9'}">class="active"</c:if>><a href="javascript:void(0);"" id="period_type_9" data-content="${session.mbrNo }" data-url="/mypage/order/indexDeliveryList?page=${orderSO.page}&rows=${orderSO.rows}&period=9&ordAcptDtmStart=<frame:timestamp date="${orderSO.ordAcptDtmStart}" dType="H" />&ordAcptDtmEnd=<frame:timestamp date="${orderSO.ordAcptDtmEnd}" dType="H" />&arrOrdDtlStatCd=${ orderSO.arrOrdDtlStatCd }"	class="bt" onclick="orderDeliveryList.setPeriod(9);return false;">최근 9개월</a></li>
												<li <c:if test="${orderSO.period eq '12'}">class="active"</c:if>><a href="javascript:void(0);"" id="period_type_12"data-content="${session.mbrNo }" data-url="/mypage/order/indexDeliveryList?page=${orderSO.page}&rows=${orderSO.rows}&period=12&ordAcptDtmStart=<frame:timestamp date="${orderSO.ordAcptDtmStart}" dType="H" />&ordAcptDtmEnd=<frame:timestamp date="${orderSO.ordAcptDtmEnd}" dType="H" />&arrOrdDtlStatCd=${ orderSO.arrOrdDtlStatCd }" class="bt" onclick="orderDeliveryList.setPeriod(12);return false;">최근 12개월</a></li>
												<li <c:if test="${orderSO.period eq '0'}">class="active"</c:if>><a href="javascript:void(0);"" id="period_type_0" data-content="${session.mbrNo }" data-url="/mypage/order/indexDeliveryList?page=${orderSO.page}&rows=${orderSO.rows}&period=0&ordAcptDtmStart=<frame:timestamp date="${orderSO.ordAcptDtmStart}" dType="H" />&ordAcptDtmEnd=<frame:timestamp date="${orderSO.ordAcptDtmEnd}" dType="H" />&arrOrdDtlStatCd=${ orderSO.arrOrdDtlStatCd }" class="bt" onclick="orderDeliveryList.setPeriod(0);return false;">직접 입력</a></li>
											</ul>
										</div>
									</nav>
