	...
	function insertAddress(){
		...
		if(checkValidateAdd()){
			...
			var options = {
				...
				, data : $("#memberAddressForm").serializeJson()
	...
	<div class="pbd">
		<div class="phd">
			<div class="in">
				...
				<button type="button" class="btnPopClose" onclick="ui.popLayer.close('addressAddPop');" data-content="" data-url="ui.popLayer.close('addressAddPop');">닫기</button>
			</div>
		</div>
		<div class="pct">
			<main class="poptents">
				<form id="memberAddressForm">
					...
					<input type ="hidden" name = "dlvrDemandYn" value ="${empty address.dlvrDemandYn ? 'N' : address.dlvrDemandYn}">
				<div class="uidelivsets">
					<ul class="list" id = "addressul">
						...
						<li class="user">
							...
							<div class="cdt">
								<div class="input del">
									<input type="text" name ="adrsNm" placeholder="이름을 입력해주세요" title="받는사람" value = "<c:out value="${address.escapedAdrsNm }" escapeXml="false"/>">
								</div>
							</div>
						</li>
						<li class="phone">
							...
							<div class="cdt">
								<div class="input del">
									<input type="text" name="mobile" placeholder="-없이 번호만 입력해주세요" title="휴대폰번호" value = "${address.mobile }">
								</div>
						...
						<li class="dreq"  data-selected ="false">
							...
							<div class="cdt">
								...
								<div class="saves">
									<div class="pp"><label class="checkbox"><input type="checkbox" name ="dftYn" value="Y" ${address.dftYn eq 'Y' ? ' checked' : ''} ${addressCnt eq 1 and not empty address.mbrDlvraNo ? ' disabled' : '' }><span class="txt"><em class="tt">기본 배송지로 등록</em></span></label></div>
								</div>
							</div>
						</li>
					</ul>
				...
				<div class="btnList bot">
						<button type="button" onclick ="ui.popLayer.close('addressAddPop');" class="btn lg d" data-dlvra-no = "${address.mbrDlvraNo }" data-content ="" data-url="ui.popLayer.close('addressAddPop');">취소</button>
					<c:if test ="${address.mbrDlvraNo ne null}">
						<button type="button" id ="addBtn" onclick =" updateAddress();" class="btn lg a" data-dlvra-no = "${address.mbrDlvraNo }" data-content ="" data-url="updateAddress();">저장</button>
					</c:if>
					<c:if test ="${address.mbrDlvraNo eq null }">
						<button type="button" id ="addBtn" onclick ="insertAddress();" class="btn lg a" data-content ="" data-url="insertAddress();">저장</button>
					</c:if>
				</div>
			</main>			
		</div>
