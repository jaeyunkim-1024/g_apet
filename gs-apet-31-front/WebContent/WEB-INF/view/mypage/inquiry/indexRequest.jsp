...
<script type="text/javascript">
	...
	function setCusCtg1Cd(obj, code, code2) {
		...
		// 주문정보 선택 노출
		if((code2 == "ORDER") && ("${session.mbrNo}" != 0)) {
			$('#orderCheck').show();
			orderFlag = "Y";
		}else {
			$('#orderCheck').hide();
	...
	var inquiryValidate = {
		all : function(){
			...
			if($("#inquiry_orderer").val().trim() == ""){
				$("#inquiry_orderer_error").html("문의자명을 입력해주세요.");
				$("#inquiry_orderer").focus();
				return false;
			} else {
				$("#inquiry_orderer_error").html("");
			}
	...
	function insertInquiry(){
		if(inquiryValidate.all()){
			...
			var options = {
					url : "<spring:url value='/mypage/inquiry/insertInquiry' />",
...
<div class="box_title_area"> 
	...
	<c:if test="${session.mbrNo eq 0}">
	<h3>비회원 1:1 문의하기</h3>
	<div class="sub_text2">궁금하신 사항을 1:1 문의에 남겨주시면, 이메일로 답변을 확인하실 수 있습니다.</div>
	</c:if>

	<c:if test="${session.mbrNo ne 0}">
	<h3>문의하기</h3>
	<div class="sub_text2">궁금하신 사항을 1:1 문의에 남겨주세요.</div>
	</c:if>

</div>

<div class="completion_box">
	<div class="ment">1:1</div>
	<div class="sub_text">
		${view.stNm}을 사랑해주시는 고객 여러분께 알려드립니다. <br />
		콜 센터와의 전화 연결은 <strong>업무 집중 시간인 오전 10시~오후 2시를 시간대를 가급적 피해 문의 주시면 더욱 원활하게 처리</strong>해 드릴 수 있습니다. <br />
		불편하시더라도 [1:1문의] 게시판을 활용해 주시면, 좀 더 신속하고 정확하게 해결해 드리도록 하겠습니다.
	</div>
	<div class="sub_box">
		...
		<div class="article">
			...
			<strong>점심시간 12:00~13:00(응대불가)</strong> 
			<strong>토, 일요일, 공휴일 휴무</strong>
		</div>
	</div>
	<div class="mgt10 t_left point3">
		<c:if test="${session.mbrNo ne 0}">
		※ 운영시간 내 문의 답변이 1시간을 초과할 경우, 해당 문의 건에 한해 <strong class="point4">적립금 2,000점</strong>을 적립해 드립니다.
		</c:if>
		<c:if test="${session.mbrNo eq 0}">
		※ 비회원 1:1 문의를 남겨주시면, 문의답변을 이메일로 확인하실 수 있습니다.
		</c:if> 
	</div>
</div>
...
<table class="table_type1">
	...
	<tbody>
		...
		<tr id="orderCheck">
			<th>주문정보</th>
			<td>
				<a href="javascript:void(0)" class="btn_h30_type4" onclick="orderData.openOderList();return false;">주문정보 선택</a>
		...
		<tr>
			<th>문의자</th>
			<td>
				<input type="hidden" name="eqrrMbrNo" value="" />
				<input type="text" id="inquiry_orderer" name="eqrrNm" class="input_style1" title="문의자" style="width:80px" value="${member.mbrNm}" <c:if test="${member ne null}">readonly="readonly"</c:if> />
				<div id="inquiry_orderer_error" class="mgt5 point1 pswd"></div>
				<div class="mgt5 point3">※ 반품 등 상품 수령 후 문의 시에는 문의자명을 정확히 남겨주시고, 문의내용에 주소지도 함께 남겨주시면 주문검색/답변에 큰 도움에 됩니다.</div>
			</td>
		</tr>
		<tr>
			<th>휴대폰 번호</th>
			<td>
				...
				<span class="mgl6 point3">※ 해피콜을 원하실 경우 반드시 연락처를 기입해주세요.</span>
		...
		<tr>
			...
			<td>
				...
				<div class="mgt5 point3">※ yahoo.co.kr, sayclub.com 등의 메일은 계정상의 문제로 메일 수신이 불가할 수 있습니다.</div>
			</td>
		</tr>
		<tr>
			<th>문의제목</th>
