	...
	<tiles:putAttribute name="script.inline">  
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				/*if(document.referrer.indexOf("/info/indexPswdUpdate") > -1){
					$('#backBtn').attr('onclick', 'history.go(-2);');
					$('#backBtn').attr('data-url', 'history.go(-2)');
				}else{
					$('#backBtn').attr('onclick', 'history.go(-1);');
					$('#backBtn').attr('data-url', 'history.go(-1)');
				}*/
			});
...
<!-- 						<div class="inr"> -->
<!-- 							<div class="hdt"> -->
<!-- 								<button id ="backBtn" class="mo-header-backNtn" onclick = "history.go(-2);" data-content="" data-url="history.go(-2)">뒤로</button> -->
							...
							ccb:function(){
								if(callback.length > 0){
									...
									if(returnUrl.indexOf("indexManageDetail") > -1){
										jQuery("<form action=\"/mypage/info/indexManageDetail\" method=\"post\"><input type=\"hidden\" name=\"checkCode\" value=\"${checkCode}\" /><input type=\"hidden\" name=\"type\" value=\"${type}\" /></form>").appendTo('body').submit();
										return;
									}
