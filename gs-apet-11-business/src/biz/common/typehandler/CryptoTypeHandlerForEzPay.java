 ...
 * - 파일명		: CryptoTypeHandlerForEzPay.java
 * - 작성일		: 2021. 03. 21.
 * - 작성자		: sorce
...
public class CryptoTypeHandlerForEzPay extends BaseTypeHandler<String> {
	...
	public String getNullableResult(ResultSet rs, String columnIndex) throws SQLException {
		return encrypt2(rs.getString(columnIndex));
	}
	...
	public String getNullableResult(CallableStatement cs, int columnIndex) throws SQLException {
		return encrypt2(cs.getString(columnIndex));
	}

	private String encrypt(String toEncrypt) {
		return cryptoService.oneWayEncrypt(toEncrypt);
	}

	private String encrypt2(String toDecrypt) {
		if (StringUtils.endsWith(toDecrypt, "=")) {
			return toDecrypt;
		} else {
			return cryptoService.oneWayEncrypt(toDecrypt);
		}
